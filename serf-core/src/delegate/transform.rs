  ...
  A: Transformable + CheapClone + core::hash::Hash + Eq + Send + Sync + 'static,
{
  ...
  fn message_encoded_len(msg: impl AsMessageRef<Self::Id, Self::Address>) -> usize {
    ...
    serf_proto::Encodable::encoded_len(&msg)
  }
  ...
  fn encode_message(
    ...
    let msg = msg.as_message_ref();
    serf_proto::Encodable::encode(&msg, dst.as_mut()).map_err(Into::into)
  }
  ...
  fn decode_message(
    ...
    match ty {
      ...
      MessageType::KeyRequest => serf_proto::KeyRequestMessage::decode(bytes.as_ref())
        .map(|(n, m)| (n, SerfMessage::KeyRequest(m)))
        .map_err(|e| Self::Error::Message(e.into())),
      #[cfg(feature = "encryption")]
      MessageType::KeyResponse => serf_proto::KeyResponseMessage::decode(bytes.as_ref())
