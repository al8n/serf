  ...
  ($rt:ident) => {
    paste::paste! {
      mod [< $rt:snake >] {
        ...
        use serf_core::tests::{event::serf_events_failed, next_socket_addr_v4, next_socket_addr_v6};
        use smol_str::SmolStr;

        #[test]
        fn test_serf_events_failed_v4() {
          let name = "serf_events_failed1_v4";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
          opts.add_bind_address(next_socket_addr_v4(0));

          let name = "serf_events_failed2_v4";
          let mut opts2 = NetTransportOptions::new(SmolStr::new(name));
          opts2.add_bind_address(next_socket_addr_v4(0));

          [< $rt:snake _run >](serf_events_failed::<
            NetTransport<
              SmolStr,
        ...
        #[test]
        fn test_serf_events_failed_v6() {
          let name = "serf_events_failed1_v6";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
          opts.add_bind_address(next_socket_addr_v6());

          let name = "serf_events_failed2_v6";
          let mut opts2 = NetTransportOptions::new(SmolStr::new(name));
