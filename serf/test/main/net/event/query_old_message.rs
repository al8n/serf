  ...
  ($rt:ident) => {
    paste::paste! {
      mod [< $rt:snake >] {
        ...
        use serf_core::tests::{event::query_old_message, next_socket_addr_v4, next_socket_addr_v6};
        use smol_str::SmolStr;

        #[test]
        fn test_query_old_message_v4() {
          let name = "query_old_message_v4";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
          opts.add_bind_address(next_socket_addr_v4(0));

          [< $rt:snake _run >](query_old_message::<
            NetTransport<
              SmolStr,
        ...
        #[test]
        fn test_query_old_message_v6() {
          let name = "query_old_message_v6";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
