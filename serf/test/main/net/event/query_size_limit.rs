  ...
  ($rt:ident) => {
    paste::paste! {
      mod [< $rt:snake >] {
        ...
        use serf_core::tests::{event::serf_query_size_limit, next_socket_addr_v4, next_socket_addr_v6};
        use smol_str::SmolStr;

        #[test]
        fn test_serf_query_size_limit_v4() {
          let name = "serf_query_size_limit_v4";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
          opts.add_bind_address(next_socket_addr_v4(0));

          [< $rt:snake _run >](serf_query_size_limit::<
            NetTransport<
              SmolStr,
        ...
        #[test]
        fn test_serf_query_size_limit_v6() {
          let name = "serf_query_size_limit_v6";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
