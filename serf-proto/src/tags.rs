...
impl Data for Tags {
    ...
    Self: Sized,
  {
    val
      .src
      .iter::<Tag>()
      .map(|res| res.and_then(|t| Tag::from_ref(t).map(Tag::split)))
  ...
  fn encoded_len(&self) -> usize {
    self
      .0
      .iter()
      .map(|(k, v)| 1 + TagRef::new(k, v).encoded_len_with_length_delimited())
      .sum::<usize>()
  }
  ...
  fn encode(&self, buf: &mut [u8]) -> Result<usize, EncodeError> {
    ...
    self
      ...
      .try_fold(0, |mut offset, (k, v)| {
        ...
        offset += 1;
        offset += TagRef::new(k, v).encode_with_length_delimited(&mut buf[offset..])?;
        Ok(offset)
      })
