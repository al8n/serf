//! Errors from constructing a [`Serf`](crate::Serf).
...
/// Why constructing a [`Serf`](crate::Serf) failed.
///
...
pub enum InitError {
  ...
  /// gossip UDP socket and the reliable TCP listener (the single-port serf
...
/// Why a [`Serf::join`](crate::Serf::join) failed.
///
...
pub enum JoinError {
  ...
  /// single-threaded by design, so its error need not cross threads.
  Resolve(Box<dyn core::error::Error + 'static>),
  /// The engine rejected the join (e.g. the node is not in the running state).
  Control(serf_embedded::SerfError),
...
impl From<serf_embedded::SerfError> for JoinError {
  fn from(e: serf_embedded::SerfError) -> Self {
    JoinError::Control(e)
  }
}
