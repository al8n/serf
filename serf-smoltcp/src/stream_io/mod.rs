//! The [`StreamIo`](serf_embedded::StreamIo) implementation over the pooled smoltcp
//! reliable-plane `tcp::Socket`s, keyed by [`SocketHandle`].
//!
//! A short-lived view, rebuilt each [`Serf::poll`](crate::Serf::poll) over the
//! already-ticked socket set and the [`Interface`] (the latter needed only by
//! `connect`, which threads the interface `context()` into smoltcp's
//! `tcp::Socket::connect`). The engine owns the reliable-plane state machine and
//! its connection pool ([`ReliablePlane`](serf_embedded::ReliablePlane)) and drives
//! the sockets entirely through this view; the stack tick (`iface.poll`) the driver
//! performs before handing the view to the engine.
//!
...
//! the engine's [`ReliablePlane`](serf_embedded::ReliablePlane) owns the
...
use serf_embedded::{StreamIo, StreamIoError};
use smoltcp::{
