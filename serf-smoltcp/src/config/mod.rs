...
/// Sizing and ports for [`Serf`](crate::Serf). All buffers are fixed-capacity at
/// construction (smoltcp has no growable backing on no_std); gossip overflow
/// drops, reliable overflow backpressures.
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct Options {
  ...
  /// single-port serf model (one advertised `SocketAddr` serves both planes,
  /// since a UDP and a TCP socket on the same port number are independent).
  pub port: u16,
  ...
  pub cidr_policy: Option<serf_embedded::CidrPolicy>,
}
...
impl Options {
  ...
  #[cfg(feature = "cidr")]
  #[cfg_attr(docsrs, doc(cfg(feature = "cidr")))]
  pub fn with_cidr_policy(mut self, policy: serf_embedded::CidrPolicy) -> Self {
