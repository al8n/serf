  ...
  ($rt:ident) => {
    paste::paste! {
      mod [< $rt:snake >] {
        ...
        use serf_core::tests::{join::join_pending_intent, next_socket_addr_v4, next_socket_addr_v6};
        use smol_str::SmolStr;

        #[test]
        fn test_join_pending_intent_v4() {
          let name = "join_pending_intent_v4";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
          opts.add_bind_address(next_socket_addr_v4(0));

          [< $rt:snake _run >](join_pending_intent::<
            NetTransport<
              SmolStr,
        ...
        #[test]
        fn test_join_pending_intent_v6() {
          let name = "join_pending_intent_v6";
          let mut opts = NetTransportOptions::new(SmolStr::new(name));
