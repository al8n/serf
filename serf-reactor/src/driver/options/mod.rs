...
impl RuntimeOptions {
  ...
  ///   delegate observation channel as a bounded flume channel; a zero-capacity
  ///   `Bounded(0)` is a rendezvous the driver's non-blocking `try_send` can
  ///   never deposit into, so every observed event would be dropped and the
  ///   delegate would never fire.
  ///
  /// `iter_drain_cap == 0` and `leave_timeout == 0` are NOT rejected: the former
  /// only caps the per-iteration batch drain (the pump still makes one-per-pass
  /// forward progress on its readiness arms and the uncapped timeout drain), and
  /// the latter is a loud immediate [`SerfError::LeaveTimeout`] rather than a
  /// silent break.
  ///
  /// There is no command-fairness knob to validate: the reactor's command queue
  /// is a `Mutex<VecDeque<Command>>` woken by a stored `Waker`, drained fully
  /// each pass, so commands never starve behind the network arms.
  #[cfg(any(feature = "tcp", feature = "quic"))]
    ...
    if matches!(self.observation_channel, Channel::Bounded(0)) {
      return Err(SerfError::InvalidOption(InvalidOption::new(
        ...
        "the delegate observation channel must have a nonzero bounded capacity: each driver \
           builds it as a bounded flume channel, and a `Bounded(0)` rendezvous is one the \
           driver's non-blocking `try_send` can never deposit into, so every observed event \
           would be dropped and the delegate would never fire"
          .to_string(),
      )));
    }
    Ok(())
...
impl StreamTransportOptions {
  ...
  ///
  /// - `dial_timeout == 0`: every outbound `StreamAction::Connect` races the
  ///   runtime's `TcpStream::connect` against `R::sleep(dial_timeout)` in a
  ///   biased select. A zero timeout makes the sleep ready immediately, so it
  ///   wins the biased select ahead of the connect and EVERY outbound dial
  ...
  /// - `bridge_inbound_cap == 0`: the per-bridge inbound channel at zero
  ///   capacity has no buffered slot, so the bridge task's non-blocking hand-off
  ///   to the driver can never deposit a frame and no reliable-stream frame
  ///   reaches the driver.
  #[cfg(feature = "tcp")]
  pub(crate) fn validate(&self) -> Result<(), SerfError> {
    if self.dial_timeout.is_zero() {
      return Err(SerfError::InvalidOption(InvalidOption::new(
        ...
        "the outbound dial budget must be nonzero: every Connect races TcpStream::connect \
           against a sleep(dial_timeout) in a biased select, so a zero timeout is ready \
           immediately and wins ahead of the connect, failing every outbound push/pull dial \
           as a spurious timeout — and since serf's join is dispatch-only, the node returns Ok \
           from join while never completing a single reliable exchange"
          .to_string(),
      )));
    }
    ...
    if self.bridge_inbound_cap == 0 {
      return Err(SerfError::InvalidOption(InvalidOption::new(
        "bridge_inbound_cap",
        "the per-bridge inbound queue capacity must be nonzero: a zero-capacity channel has no \
           buffered slot, so the bridge task's non-blocking hand-off to the driver can never \
           deposit a frame and no reliable-stream frame reaches the driver"
          .to_string(),
      )));
    }
    Ok(())
