...
async fn wildcard_enumerates_interface_addrs() {
  ...
  // The wildcard was replaced by real interface addresses, all on the asked port;
  // none is the wildcard itself.
...
async fn wildcard_respects_address_family() {
  let r = LocalAddrResolver::all();
  // `0.0.0.0` must never substitute an IPv6 address, and `[::]` never an IPv4 one —
  // the resolved bind+advertise address keeps the requested family.
...
fn local_advertise_attaches_port() {
  ...
  // the helper attaches the requested port.
...
fn public_scope_excludes_special_purpose() {
  ...
  // Real global unicast inside 2001::/16 — must be public (not the whole /16
  // special-purpose block; only 2001::/23 + 2001:db8::/32 are).
