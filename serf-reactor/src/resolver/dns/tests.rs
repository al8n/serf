...
/// The tokio-backed DNS resolver under test.
type Dns = DnsResolver<TokioRuntime>;

/// The DNS resolver's user-facing address type — the hostname-or-IP type the OS
/// resolver also consumes.
type Address = HostAddr<SmolStr>;

/// A nameserver that is guaranteed never to answer: `240.0.0.1` is in the reserved
/// 240.0.0.0/4 block (RFC 1112 §4) and is unroutable. Pairing it with a fast-path
/// test proves the IP-literal branch never opens a socket — a real connect would
/// hang well past any test budget.
...
/// timeout, so any test that *did* hit the network would fail loudly rather than
/// silently fall through.
fn resolver_with_dead_server() -> Dns {
  Dns::from_servers(vec![dead_server()]).with_timeout(Duration::from_millis(50))
}

/// Allocate a unique temp path for a resolv.conf fixture. Avoids a `tempfile`
/// dev-dependency; the counter + PID keep parallel test threads from colliding.
fn temp_resolv_conf_path(tag: &str) -> PathBuf {
  static COUNTER: AtomicU64 = AtomicU64::new(0);
  let n = COUNTER.fetch_add(1, Ordering::Relaxed);
  let mut p = std::env::temp_dir();
  p.push(format!(
    "serf-reactor-dns-{tag}-{}-{n}.conf",
    std::process::id()
  ));
  p
}
...
fn from_servers_uses_default_timeout() {
  let r = Dns::from_servers(vec![dead_server()]);
...
fn from_empty_servers_uses_default_timeout() {
  let r = Dns::from_servers(Vec::new());
  assert_eq!(r.timeout(), DEFAULT_DNS_TIMEOUT);
}
...
fn with_timeout_overrides_default() {
  ...
  let r = Dns::from_servers(vec![dead_server()]).with_timeout(custom);
...
fn from_resolv_conf_parses_valid_file() {
  ...
  let r = Dns::from_resolv_conf(&path).expect("parse valid resolv.conf");
  // No accessor exposes the parsed server list, but a successful parse must still
  // leave the default per-query timeout in place.
  assert_eq!(r.timeout(), DEFAULT_DNS_TIMEOUT);

  // Ignoring Err: best-effort cleanup of the temp fixture; a leaked file in the OS
  // temp dir is harmless and must not fail the test.
...
fn from_resolv_conf_parses_empty_file() {
  ...
  let r = Dns::from_resolv_conf(&path).expect("parse empty resolv.conf");
  assert_eq!(r.timeout(), DEFAULT_DNS_TIMEOUT);
...
fn from_resolv_conf_rejects_invalid_nameserver() {
  ...
  // `DnsResolver` is not `Debug`, so map the `Ok` side to `()` before `expect_err`
  // can report it.
  let err = Dns::from_resolv_conf(&path)
    .map(|_| ())
...
async fn resolve_ipv4_literal_skips_dns() {
  // An IPv4 literal must short-circuit before any socket is opened, even though the
  // resolver is pointed at an unroutable nameserver.
...
async fn resolve_ip_literal_without_port_uses_zero() {
  // `HostAddr::port()` is `None` for a bare IP; the resolver substitutes 0.
  let r = resolver_with_dead_server();
...
async fn resolve_ip_literal_works_with_empty_server_list() {
  // The fast path does not consult `self.servers`, so an empty list is irrelevant
  // for IP literals.
  let r = Dns::from_servers(Vec::new());
...
async fn resolve_short_name_bypasses_tcp_and_uses_os_fallback() {
  // `localhost` has no `.`, so the TCP-first branch is skipped entirely and
  // resolution goes straight to the OS resolver. `localhost` maps to loopback via
  // the hosts file, so this stays off the external network even though the
  // configured nameserver is unroutable.
  let r = resolver_with_dead_server();
...
fn dns_error_into_io_error_roundtrips() {
  // `From<DnsError> for io::Error` wraps via `io::Error::other`, so the kind is
  // `Other` and the source string is preserved.
...
fn dns_error_decode_variant_displays() {
  // Exercise the `Decode` variant's `Display` prefix by forcing a decode failure
  // from a too-short DNS message buffer.
...
/// A loopback TCP server that speaks just enough of the TCP-DNS wire protocol (RFC
/// 1035 §4.2.2) to answer ONE query: read the 2-byte length prefix + query body,
/// then write back a framed DNS response carrying the supplied A and AAAA answers.
/// Returns the bound address so the resolver can target it. Stays entirely on
/// loopback — no external DNS is contacted.
///
...
/// validating the question, so a fixed answer set exercises the full frame →
/// connect → write → read-length → read-body → decode → collect path.
async fn spawn_tcp_dns_server(answers: Vec<RData>) -> (SocketAddr, tokio::task::JoinHandle<()>) {
  ...
  let handle = tokio::spawn(async move {
    ...
    // Build a response message carrying the fixed answers. The id is irrelevant to
    // the resolver (it does not match the query id).
    ...
    // Ignoring Err: best-effort single write into a test fixture; if the client
    // hung up the resolver test will surface the failure itself.
...
// The TCP-first success path: a FQDN (contains a `.`) with a configured nameserver
// drives `tcp_query` → `tcp_query_inner` end to end against a loopback DNS server,
// and the harvested A + AAAA answers (carrying the query port) are returned without
// consulting the OS fallback.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn tcp_query_collects_a_and_aaaa_from_loopback_server() {
  ...
  let answers = vec![
    ...
    // A CNAME-like non-address record is ignored by the collector; model it with a
    // TXT to prove the `_ => {}` arm is taken.
  ...
  let r = Dns::from_servers(vec![server_addr]).with_timeout(Duration::from_secs(5));
  ...
  // Both address answers surface, each carrying the requested port; the TXT is
  // dropped.
  assert_eq!(
    resolved.len(),
    2,
...
// When the TCP-DNS server returns an EMPTY answer set, the resolver treats the TCP
// attempt as unproductive and falls through to the OS fallback. With a short
// loopback name target the fallback resolves loopback, proving the empty-TCP-answer
// → fallback path (the `!addrs.is_empty()` guard going false) rather than surfacing
// the empty TCP result.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn tcp_query_empty_answers_falls_through_to_os_fallback() {
  ...
  // branch is attempted; the empty answer makes it fall through to the OS resolver,
  // which maps localhost to loopback.
