  ...
  R: Runtime,
{
  ...
  /// budget. On timeout returns [`DnsError::Io`]`(io::ErrorKind::TimedOut)`; the
  /// resolver's caller falls back to the OS resolver via the standard error path
  /// (see [`Resolver::resolve`] below).
  ...
  fn resolve(
    ...
    async move {
      ...
      if host_str.contains('.') && !self.servers.is_empty() {
        // Ignoring Err: TCP-first is best-effort per the upstream spec ("If this
        // fails it's not fatal since this isn't a standard way to query DNS, and we
        // have a fallback below.", memberlist.go:404). We unconditionally fall
        // through to the OS resolver on any error or empty answer.
