...
use super::{KeyringDelegate, KeyringPersistError, KeyringPersistence};
...
/// back through [`KeyringPersistence::Pending`] so the driver can gate the
...
impl KeyringDelegate for FileKeyringDelegate {
  fn keyring_updated(&self, keyring: &Keyring) -> KeyringPersistence {
    // Non-blocking hand-off: the pump must never wait on storage. A failed
    // send (the worker already hung up) drops the job — and with it the ack
    // sender — so the returned receiver disconnects and the pump reports the
    // rotation unpersisted rather than silently acknowledged.
    ...
    KeyringPersistence::Pending(rx)
  }
}

impl Drop for FileKeyringDelegate {
  fn drop(&mut self) {
