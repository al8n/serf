...
//!
//! The serf driver is `no_std + alloc`, so the firmware must register a global
//! allocator. [`embedded_alloc::LlffHeap`] manages a fixed static byte region;
//! [`init`] hands it that region once at startup.
...
///
/// Sized for two embassy-net stacks plus two serf engines (the serf super-machine
/// over memberlist: gossip codec scratch, the reliable-plane connection state,
/// membership maps, and serf's event/query bookkeeping) running concurrently on
/// the AN386's 4 MB RAM. Generous because the machine has the
