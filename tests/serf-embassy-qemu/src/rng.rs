...
//! so a fixed seed makes every run identical. (Each node additionally receives its
//! own gossip and serf RNGs as seeded `SmallRng`s passed to `Serf::new_with_rng`,
