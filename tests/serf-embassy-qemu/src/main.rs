...
//! thread-mode executor on a SysTick time driver, and runs two serf nodes over an
//! in-memory paired embassy-net link until they converge on a two-member view AND
//! a user event broadcast by one node is observed by the other — then exits 0 via
//! semihosting. A deadline guard exits non-zero if either step stalls, so the qemu
//! process exit code is the pass/fail signal.
//!
//! This is the runtime counterpart to the host loopback / user-event tests in
//! `serf-embassy/tests/`: same two-node join and user-event crossing, but on a
//! real emulated core with no `std`.
...
fn main() -> ! {
  ...
  executor.run(|spawner| {
    ...
    // (two serf Runners + two embassy-net Runners) once it has built them.
