...
use super::{
  Profile,
  event_handler::{EventScript, parse_event_script},
};
...
impl<I, A> AgentOptions<I, A> {
  ...
  /// Returns the list of EventScripts associated with this
  /// configuration and specified by the "event_handlers" configuration.
  pub fn event_scripts(&self) -> impl Iterator<Item = EventScript> + Send + '_ {
    self
      .event_handlers
      .iter()
