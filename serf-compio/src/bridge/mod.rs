...
///
/// All `Sender::send` failures are ignored with a justification: if the driver
/// dropped the inbound receiver we are already shutting down and the event can
/// be discarded.
          ...
          Some(BridgeOut::Bytes(bytes)) => {
            ...
            match write_cancellable(&mut write_half, bytes, &mut cancel_fut, close_timeout).await {
              ...
              WriteStatus::Wrote(res) => {
                if res.is_err() {
                  // The io::Error is not surfaced — `handle_transport_error`
                  // keys only on the exchange id.
    ...
    select_biased! {
      ...
      recv_msg = recv_fut => {
        ...
        match res {
          ...
          Ok(n) => {
            // Tight copy into a right-sized Vec; the original `recv_buf_len`
            // `buf` drops here.
          ...
          Err(_err) => {
            // The io::Error is not surfaced — `handle_transport_error` keys
            // only on the exchange id.
            // Ignoring Err: shutdown-race reasoning.
            let _ = inbound_tx
              .send(BridgeInbound::Error(BridgeError { eid, received_at }))
              .await;
            break;
          }
        }
