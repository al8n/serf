...
// A `Bounded(0)` observation channel would panic the driver's `lochan::mpsc`
// rendezvous-free channel; `validate` rejects it at construction instead.
...
// A zero `cmd_fairness_budget` disables the only drain that keeps user commands
// progressing under a continuous inbound flood, so shutdown / leave / joins
// would hang; `validate` rejects it at construction.
#[cfg(any(feature = "tcp", feature = "quic"))]
#[test]
fn runtime_validate_rejects_zero_cmd_fairness_budget() {
  let opts = RuntimeOptions::new().with_cmd_fairness_budget(0);
  assert!(matches!(
    opts.validate(),
    Err(crate::SerfError::InvalidOption(_))
  ));
...
// so they are accepted rather than rejected.
#[cfg(any(feature = "tcp", feature = "quic"))]
#[test]
...
// serde — a numeric `cmd_fairness_budget` and the humantime `idle_wake_interval`
// duration parse, then fail validation.
#[cfg(all(feature = "serde", any(feature = "tcp", feature = "quic")))]
#[test]
fn runtime_validate_rejects_zero_cmd_fairness_budget_from_serde() {
  let opts: RuntimeOptions =
    serde_json::from_str(r#"{"cmd_fairness_budget":0}"#).expect("deserialize");
  assert_eq!(opts.cmd_fairness_budget(), 0);
  assert!(matches!(
    opts.validate(),
    Err(crate::SerfError::InvalidOption(_))
  ));
...
#[cfg(all(feature = "clap", any(feature = "tcp", feature = "quic")))]
#[test]
fn runtime_validate_rejects_zero_cmd_fairness_budget_from_clap() {
  use clap::Parser;
  ...
  let cli = Cli::try_parse_from(["app", "--runtime-cmd-fairness-budget", "0"])
    .expect("clap parses cmd-fairness-budget 0");
  assert_eq!(cli.runtime.cmd_fairness_budget(), 0);
  assert!(matches!(
    cli.runtime.validate(),
    Err(crate::SerfError::InvalidOption(_))
  ));
...
fn runtime_options_partial_update_preserves_unset_fields() {
  ...
  assert_eq!(cli.o.cmd_fairness_budget(), 5);
