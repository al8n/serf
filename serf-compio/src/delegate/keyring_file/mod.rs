...
use serf_driver::{KeyringFilePersistence, KeyringPersistence};
...
/// delegate, is Unix-only). The engine's persistence worker is a plain
/// thread, so the delegate drops into compio's single-threaded driver world
/// without carrying any `Send` requirement onto the pump.
