...
fn acked(rx: crate::keyring::KeyringPersistRx) -> Result<(), KeyringPersistError> {
...
/// Any sibling temp file this engine could have produced for `path`: the
...
fn rotation_round_trips_through_the_file() {
  ...
  acked(engine.enqueue(&ring)).expect("the rotation persists");
...
fn missing_file_loads_as_none() {
  let engine = KeyringFilePersistence::new(tmp_path("missing"));
  assert!(engine.load().expect("not-found is not an error").is_none());
}
...
fn malformed_files_are_parse_errors() {
  for (name, contents) in [
    ...
    let engine = KeyringFilePersistence::new(&path);
    assert!(
      matches!(engine.load(), Err(KeyringFileError::Parse(_))),
...
fn a_second_rotation_replaces_the_first() {
  ...
  acked(engine.enqueue(&Keyring::new(first))).expect("first rotation persists");
  acked(engine.enqueue(&Keyring::new(second))).expect("second rotation persists");
...
fn construction_sweeps_stale_temps() {
  ...
  let _delegate = KeyringFilePersistence::new(&path);
  assert!(
    !legacy.exists(),
...
fn a_planted_symlink_never_receives_key_bytes() {
  ...
  acked(engine.enqueue(&Keyring::new(key))).expect("rotation persists");
  assert_eq!(
    std::fs::read_to_string(&victim).expect("victim readable"),
    "victim contents\n",
...
fn a_tmp_extension_destination_survives_construction() {
  ...
  {
    let engine = KeyringFilePersistence::new(&path);
    acked(engine.enqueue(&Keyring::new(key))).expect("the rotation persists");
  }

  let reopened = KeyringFilePersistence::new(&path);
  let loaded = reopened
    .load()
    .expect("the persisted keyring parses")
    .expect("constructing a engine must not sweep a .tmp-named destination");
...
fn an_uppercase_tmp_destination_survives_construction() {
  ...
  let loaded = reopened
    .load()
    .expect("the persisted keyring parses")
    .expect("constructing a engine must not sweep a case-aliased .TMP destination");
  assert_eq!(loaded.primary_ref(), &key);
...
fn a_symlinked_destination_keeps_its_target_through_construction() {
  ...
  let key = SecretKey::ChaCha20Poly1305([13u8; 32]);
  {
    ...
    let seed = KeyringFilePersistence::new(&target);
    acked(seed.enqueue(&Keyring::new(key))).expect("the seed rotation persists");
  }
  std::os::unix::fs::symlink(&target, &link).expect("plant the destination symlink");

  let engine = KeyringFilePersistence::new(&link);
  let loaded = engine
    .load()
...
fn an_unsyncable_directory_fails_the_acknowledgement() {
  ...
  let outcome = acked(engine.enqueue(&Keyring::new(key)));
...
fn persistence_failure_is_acknowledged_as_an_error() {
  ...
  assert!(
    acked(engine.enqueue(&Keyring::new(key))).is_err(),
    "a write into a missing directory must acknowledge failure"
  );
}

/// Dropping the engine joins the worker after it drains the queue: a
