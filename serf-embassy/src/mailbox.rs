...
//! [`StreamIo`](serf_embedded::StreamIo) to an asynchronous embassy-net
//! `TcpSocket`.
//!
...
//! [`pump`](serf_embedded::SerfEngine::pump) is synchronous — every mailbox
...
/// A directive the engine (via [`SerfStream`](crate::stream_io::SerfStream))
...
pub(crate) enum Command {
  /// No pending directive. A free slot defaults to this; the worker parks until
  /// the engine posts a `Listen` / `Dial` (see [`crate::worker`]).
  Idle,
...
pub(crate) struct Mailbox {
  ...
  /// [`SerfStream::reuse_ready`](crate::stream_io::SerfStream) so it never
