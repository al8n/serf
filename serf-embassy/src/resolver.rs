...
//! advertise address) and [`join`](crate::Serf::join) (the seeds) — so a resolver
//! is passed to those methods rather than stored on the [`Serf`](crate::Serf)
//! handle. Everything past the boundary is a wire [`SocketAddr`]; the embedded
//! engine only ever sees resolved addresses.

use core::net::SocketAddr;

use serf_embedded::ResolvedAddrs;
...
/// [`MAX_RESOLVED_ADDRS_PER_SEED`](serf_embedded::MAX_RESOLVED_ADDRS_PER_SEED).
