//! The [`StreamIo`](serf_embedded::StreamIo) implementation over the per-slot
//! mailboxes, keyed by [`SlotId`].
//!
...
//! the new directive.
//!
//! Because the engine's [`pump`](serf_embedded::SerfEngine::pump) is
...
impl StreamIo for SerfStream<'_> {
  type Conn = SlotId;
