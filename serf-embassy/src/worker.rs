...
//! engine's synchronous [`StreamIo`](serf_embedded::StreamIo) view to read.
//!
//! # Borrow discipline
//!
...
/// design: the shared engine ([`serf_embedded::SerfEngine`]) tracks exactly ONE
...
pub(crate) async fn run_slot(
  ...
  // deadlines at construction (see `Serf::new`), so it installs directly.
  ...
  loop {
    ...
    match select(socket.read(&mut tmp[..cap]), cmd_wake.wait()).await {
      ...
      Either::First(Err(_)) => {
        ...
        // here would make `SerfStream::recv_finished` (`peer_fin &&
        ...
        // SUCCESSFUL completion — so a mid-send RST would falsely resolve an
        // exchange as Ok. Surfacing only `open = false` keeps the reset a failure:
        // the exchange's bridge elapses at its `stream_timeout` deadline and the
        // machine emits `StreamAction::Abort`, exactly as the smoltcp driver treats
        // a RST-`Closed` socket (its `recv_finished` likewise excludes the reset
        // state). Any already-buffered inbound still drains via `recv` before then;
        // it simply is not followed by a spurious EOF anchor.
        {
          let mut m = mb.borrow_mut();
          m.open = false;
          m.sock_send_queue = 0;
